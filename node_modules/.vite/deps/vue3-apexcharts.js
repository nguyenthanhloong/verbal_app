...
var Defaults = class {
  ...
  _getBoxTooltip(w, seriesIndex, dataPointIndex, labels, chartType) {
    ...
    if (w.config.series[seriesIndex].type && w.config.series[seriesIndex].type !== chartType) {
      ...
      return `<div class="apexcharts-tooltip-box apexcharts-tooltip-${w.config.chart.type}"><div>${labels[0]}: <span class="value">` + o + `</span></div><div>${labels[1]}: <span class="value">` + h2 + "</span></div>" + (m ? `<div>${labels[2]}: <span class="value">` + m + "</span></div>" : "") + `<div>${labels[3]}: <span class="value">` + l + `</span></div><div>${labels[4]}: <span class="value">` + c + "</span></div></div>";
    }
  }
};
var name = "en";
...
var Graphics = class _Graphics {
  ...
  arc(rx, ry, axisRotation, largeArcFlag, sweepFlag, x2, y, relative = false) {
    let coord = "A";
    if (relative) coord = "a";
    const arc = [coord, rx, ry, axisRotation, largeArcFlag, sweepFlag, x2, y].join(
      " "
    );
    return arc;
  }
  ...
  drawMarkerShape(x2, y, type, size, opts) {
    const path = this.drawPath({
      d: this.getMarkerPath(x2, y, type, size, opts),
...
var Scatter = class {
  ...
  drawPoint(x2, y, radius, realIndex, dataPointIndex, j) {
    const w = this.w;
    const i = realIndex;
    ...
    const el = graphics.drawMarker(x2, y, markerConfig);
    if (w.config.series[i].data[dataPointIndex]) {
      if (w.config.series[i].data[dataPointIndex].fillColor) {
...
var DataLabels = class {
  ...
  dataLabelsCorrection(x2, y, val, i, dataPointIndex, alwaysDrawDataLabel, fontSize) {
    const w = this.w;
    const graphics = new Graphics(this.w);
    ...
    w.globals.dataLabelsRects[i].push({ x: x2, y, width, height });
  ...
  drawDataLabel({ type, pos, i, j, isRangeStart, strokeWidth = 2 }) {
    ...
    for (let q = 0; q < pos.x.length; q++) {
      x2 = pos.x[q] + dataLabelsConfig.offsetX;
  ...
  plotDataLabelsText(opts) {
    ...
    if (correctedLabels.textRects) {
      if (x2 < -20 - correctedLabels.textRects.width || x2 > w.layout.gridWidth + correctedLabels.textRects.width + 30) {
        text = "";
      }
    }
...
var AxesUtils = class {
  ...
  getLabel(labels, timescaleLabels, x2, i, drawnLabels = [], fontSize = "12px", isLeafGroup = true) {
    const w = this.w;
...
var XAxis = class {
  ...
  drawXAxisLabelAndGroup(isLeafGroup, graphics, elXaxisTexts, labels, isXNumeric, colWidthCb, overwriteStyles = {}) {
    ...
    for (let i = 0; i <= labelsLen - 1; i++) {
      let x2 = xPos - colWidthCb(i, colWidth) / 2 + w.config.xaxis.labels.offsetX;
...
var YAxis = class {
  ...
  drawYaxisInversed(realIndex) {
    ...
    if (w.config.xaxis.labels.show) {
      for (let i = timescaleLabels.length ? 0 : tickAmount; timescaleLabels.length ? i < timescaleLabels.length : i >= 0; timescaleLabels.length ? i++ : i--) {
        ...
        let x2 = w.layout.gridWidth + w.globals.padHorizontal - (l - labelsDivider + w.config.xaxis.labels.offsetX);
  ...
  yAxisTitleRotate(realIndex, yAxisOpposite) {
    ...
    if (yAxisTitle) {
      ...
      yAxisTitle.setAttribute("x", x2.xPos - (yAxisOpposite ? 10 : 0));
...
var Series = class _Series {
  ...
  highlightSeries(seriesName) {
    ...
    for (let se2 = 0; se2 < allSeriesEls.length; se2++) {
      allSeriesEls[se2].classList.add(this.legendInactiveClass);
    }
    if (seriesEl) {
      if (!w.globals.axisCharts) {
      ...
      for (let se2 = 0; se2 < allSeriesEls.length; se2++) {
        allSeriesEls[se2].classList.remove(this.legendInactiveClass);
      }
    }
  }
    ...
    if (heatTreeSeries.length > 0) {
      for (let h2 = 0; h2 < heatTreeSeries.length; h2++) {
        ...
        for (let i = 0; i < seriesEls.length; i++) {
          const getAttr = (x2) => {
            return seriesEls[i].getAttribute(x2);
          };
          const rect = {
    ...
    for (let j = 0; j < activeData.length; j++) {
      ...
      if (isXString || isXDate) {
        if (isXString || cnf.xaxis.convertedCatToNumeric) {
          ...
          if (cnf.xaxis.type === "datetime") {
            this.twoDSeriesX.push(dt.parseDate(x2.toString()));
          } else {
            this.w.axisFlags.dataFormatXNumeric = true;
            this.w.axisFlags.isXNumeric = true;
            this.twoDSeriesX.push(parseFloat(x2));
          }
        }
      } else if (isXArr) {
        this.fallbackToCategory = true;
...
var Position = class {
  ...
  moveXCrosshairs(cx, j = null) {
    ...
    if (j !== null) {
      x2 = w.layout.gridWidth / tickAmount * j;
    }
    if (xcrosshairs !== null && !w.globals.isBarHorizontal) {
      xcrosshairs.setAttribute("x", x2);
      xcrosshairs.setAttribute("x1", x2);
      xcrosshairs.setAttribute("x2", x2);
  ...
  moveTooltip(cx, cy, markerSize = null) {
    ...
    let x2 = parseFloat(cx) + pointSize + 5;
    let y = parseFloat(cy) + pointSize / 2;
    if (x2 > w.layout.gridWidth / 2) {
      x2 = x2 - tooltipRect.ttWidth - pointSize - 10;
    }
    if (x2 > w.layout.gridWidth - tooltipRect.ttWidth - 10) {
      x2 = w.layout.gridWidth - tooltipRect.ttWidth;
    }
    if (x2 < -20) {
      x2 = -20;
    }
    if (w.config.tooltip.followCursor) {
      ...
      const seriesBound = elGrid.getBoundingClientRect();
      x2 = ttCtx.e.clientX - seriesBound.left;
      if (x2 > w.layout.gridWidth / 2) {
...
var Intersect = class {
  ...
  handleHeatTreeTooltip({ e, opt, x: x2, y, type }) {
    const ttCtx = this.ttCtx;
    const w = this.w;
    if (e.target.classList.contains(`apexcharts-${type}-rect`)) {
      ...
      w.interact.capturedDataPointIndex = j;
      x2 = cx + ttCtx.tooltipRect.ttWidth / 2 + width;
      ...
      if (ttCtx.w.config.tooltip.followCursor) {
        ...
        x2 = w.interact.clientX - seriesBound.left - (x2 > w.layout.gridWidth / 2 ? ttCtx.tooltipRect.ttWidth : 0);
   ...
   */
  handleMarkerTooltip({ e, opt, x: x2, y }) {
    const w = this.w;
    const ttCtx = this.ttCtx;
    let i;
    let j;
    if (e.target.classList.contains("apexcharts-marker")) {
      ...
      ttCtx.marker.enlargeCurrentPoint(j, opt.paths, x2, y);
    }
    return {
      x: x2,
      y
  ...
  handleBarTooltip({ e, opt }) {
    ...
    if (!ttCtx.fixedTooltip && (!w.config.tooltip.shared || w.globals.isBarHorizontal && ttCtx.tooltipUtil.hasBars())) {
      ...
      tooltipEl.style.left = x2 + w.layout.translateX + "px";
      tooltipEl.style.top = y + "px";
    }
  }
  getBarTooltipXY({ e, opt }) {
    ...
    if (cl.contains("apexcharts-bar-area") || cl.contains("apexcharts-candlestick-area") || cl.contains("apexcharts-boxPlot-area") || cl.contains("apexcharts-rangebar-area")) {
      ...
      const handleXForColumns = (x22) => {
        if (w.axisFlags.isXNumeric) {
          ...
          if (this.isVerticalGroupedRangeBar) {
            ...
            x22 = cx - ttCtx.dataPointsDividedWidth + bw / 2;
          }
        }
        return x22;
      };
      ...
      if (w.config.tooltip.followCursor) {
        ...
        if (w.globals.isBarHorizontal) {
          x2 = cx;
          if (x2 < ttCtx.xyRatios.baseLineInvertedY) {
  ...
  nonAxisChartsTooltips({ e, opt, tooltipRect }) {
    ...
    if (e.type === "mousemove" || e.type === "touchmove") {
      ...
      if (w.config.tooltip.intersect && arcPath.hasAttribute("data:cx") && arcPath.hasAttribute("data:cy")) {
        ...
        x2 = svgBound.left - seriesBound.left + parseFloat(arcPath.getAttribute("data:cx")) - tooltipRect.ttWidth / 2;
        y = svgBound.top - seriesBound.top + parseFloat(arcPath.getAttribute("data:cy")) - tooltipRect.ttHeight - 10;
      } else {
        x2 = w.interact.clientX - seriesBound.left - tooltipRect.ttWidth / 2;
...
var SVGFilter = class extends SVGElement {
  ...
  size(w, h2, x2, y) {
    return this.attr({ width: w, height: h2, x: x2, y });
  }
};
...
function parsePath(d) {
  ...
  const commands = [];
  const re2 = /([MmLlHhVvCcSsQqTtAaZz])\s*/g;
  const numRe = /[+-]?(?:\d+\.?\d*|\.\d+)(?:e[+-]?\d+)?/gi;
  let match;
  const letters = [];
  const positions = [];
  while ((match = re2.exec(d)) !== null) {
...
function arcToBezier(pos, val) {
  var rx = Math.abs(val[1]), ry = Math.abs(val[2]), xAxisRotation = val[3] % 360, largeArcFlag = val[4], sweepFlag = val[5], x2 = val[6], y = val[7], A = new Point(pos[0], pos[1]), B = new Point(x2, y), primedCoord, lambda, mat, k, c, cSquare, t, O, OA, OB, tetaStart, tetaEnd, deltaTeta, nbSectors, f, arcSegPoints, angle, sinAngle, cosAngle, pt, i, il, retVal = [], x1, y1, x22, y2;
...
var ZoomPanSelection = class extends Toolbar {
    ...
    if (!w.interact.zoomEnabled) {
      if (typeof w.interact.selection !== "undefined" && w.interact.selection !== null) {
        ...
        if (w.config.chart.selection.xaxis.min !== void 0 && w.config.chart.selection.xaxis.max !== void 0) {
          let x2 = (w.config.chart.selection.xaxis.min - w.globals.minX) / xyRatios.xRatio;
          let width = w.layout.gridWidth - (w.globals.maxX - w.config.chart.selection.xaxis.max) / xyRatios.xRatio - x2;
          if (w.axisFlags.isRangeBar) {
  ...
  selectionDragging(type, e) {
    ...
    let { x: x2, y } = box;
    if (x2 < this.constraints.x) {
      x2 = this.constraints.x;
    }
    if (y < this.constraints.y) {
      y = this.constraints.y;
    }
    if (box.x2 > this.constraints.x2) {
      x2 = this.constraints.x2 - box.w;
    }
...
var Helpers$2 = class Helpers3 {
  ...
  getStringX(x2) {
    ...
    if (w.config.xaxis.convertedCatToNumeric && w.labelData.categoryLabels.length) {
      x2 = w.labelData.categoryLabels.indexOf(x2) + 1;
    }
    const catIndex = w.labelData.labels.map((item) => Array.isArray(item) ? item.join(" ") : item).indexOf(x2);
    const xLabel = w.dom.baseEl.querySelector(
...
var BarDataLabels = class {
  ...
  handleBarDataLabels(opts) {
    ...
    if (w.axisFlags.isXNumeric && !w.globals.isBarHorizontal) {
      bcx = x2 + parseFloat(barWidth * (visibleSeries + 1));
      bcy = y + parseFloat(barHeight * (visibleSeries + 1)) - strokeWidth;
    } else {
      bcx = x2 + parseFloat(barWidth * visibleSeries);
  ...
  drawTotalDataLabels({
    ...
    if (barTotalDataLabelsConfig.enabled && typeof x2 !== "undefined" && typeof y !== "undefined" && this.barCtx.lastActiveBarSerieIndex === realIndex) {
...
var Bar = class {
  ...
  drawBarPaths({
    ...
    x2 = this.barHelpers.getXForValue(this.series[i][j], zeroW);
  ...
  getBarXForNumericXAxis({ x: x2, barWidth, realIndex, j }) {
    ...
    if (Utils$1.isNumber(w.seriesData.seriesX[sxI][j])) {
      x2 = (w.seriesData.seriesX[sxI][j] - w.globals.minX) / this.xRatio - barWidth * this.seriesLen / 2;
    }
    return {
      barXPosition: x2 + barWidth * this.visibleI,
      x: x2
...
var BarStacked = class extends Bar {
  draw(series, seriesIndex) {
    ...
    for (let i = 0, bc = 0; i < series.length; i++, bc++) {
      ...
      const initPositions = this.initialPositions(x2, y, void 0, void 0, void 0, void 0, translationsIndex);
      const {
        xDivision,
        // xDivision is the GRIDWIDTH divided by number of datapoints (columns)
        yDivision,
  ...
  drawStackedBarPaths({
    ...
    if (this.series[i][j] === null) {
      ...
      x2 = barXPosition + this.series[i][j] / this.invertedYRatio - (this.isReversed ? this.series[i][j] / this.invertedYRatio : 0) * 2;
    }
    const paths = this.barHelpers.getBarpaths({
      barYPosition,
      barHeight,
...
var BoxCandleStick = class extends Bar {
  ...
  getOHLCValue(i, j) {
    ...
    const h2 = getCandleVal(w.candleData.seriesCandleH);
...
var HeatMap = class {
  ...
  animateHeatMap(el, x2, y, width, height, speed) {
...
var Line = class {
  ...
  _iterateOverDataPoints({
    ...
    for (let j = 0; j < iterations; j++) {
      ...
      xArrj.push(series[i][j + 1] === null ? null : x2);
  ...
  _createPaths({
    ...
    switch (curve) {
      ...
      default: {
        const pathToPoint = (curve2, x22, y3) => {
          let path = [];
          switch (curve2) {
            case "stepline":
              path = graphics.line(x22, null, "H") + graphics.line(null, y3, "V");
              break;
            case "linestep":
              path = graphics.line(null, y3, "V") + graphics.line(x22, null, "H");
              break;
            case "straight":
              path = graphics.line(x22, y3);
              break;
          }
          return path;
        };
...
var RangeBar = class extends Bar {
  ...
  detectOverlappingBars({
    ...
    const x2 = w.config.series[i].data[j].x;
...
var ae = defineComponent({
  ...
  setup(t) {
    ...
    return onServerPrefetch(async () => {
      try {
        const { default: r } = await import("./apexcharts.ssr.esm-fe46cd2d-37UVWDMT.js"), n = Object.assign({}, t.options, {
...
var P = defineComponent({
  ...
  setup(t) {
    ...
    onMounted(async () => {
      try {
        const { default: r } = await import("./apexcharts.ssr.esm-fe46cd2d-37UVWDMT.js");
